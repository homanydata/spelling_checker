...
class LanguageMaster:
    ...
    def suggest(self, word: str, n: int=3) -> list[str]:
        ...
        suggestions.sort(key=lambda w: edit_distance(word1=word, word2=w, limit=LanguageMaster.DISTANCE_LIMIT), reverse=True)
    ...
    def get_mistakes(self, text: str) -> list[str]:
        ...
        mistakes = list(filter(lambda w: not check_spelling(w), words))
        return mistakes
    ...
    def create_correction(text: str) -> str:
        ...
        for mistake in mistakes:
            correction = self.suggest(word=mistake, n=1)
            corrected_text.replace(mistake, correction)
